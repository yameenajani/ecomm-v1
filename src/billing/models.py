...
class ChargeManager(models.Manager):
    def do(self, billing_profile, order_obj, card=None): # Charge.objects.do()
        ...
        new_charge_obj.save()
        if new_charge_obj.paid:
            context = {
                'order_id': order_obj.order_id,
                'total': order_obj.total,
                'card': card_obj.last4,
                'payment_status': new_charge_obj.seller_message,
            }
            txt_ = get_template("transaction/trans.txt").render(context)
            html_ = get_template("transaction/trans.html").render(context)
